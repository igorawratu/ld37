...
public class ShaderNode
{
	...
	public ShaderNode(string shader_name, uint width, uint height, bool highres, bool pointSampling)
    {
		...
		for (int i = 0; i < 2; ++i)
        {
			...
			if(pointSampling)
				output_ [i].filterMode = FilterMode.Point;
        }
    ...
    public virtual void Execute()
    {
        ...
        foreach(var texture in input_textures_)
        {
			...
			Debug.Log (shader_.name+" "+texture.Key + "_TexelSize "+ texelsize);
		}
